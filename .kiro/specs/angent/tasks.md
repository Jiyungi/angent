...
- [ ] 0. Repository and version-control setup
  - [-] 0.1 Initialize Git and connect the GitHub remote
  ...
  - [~] 0.2 Author `.gitignore` to keep secrets and artifacts untracked
  ...
  - [~] 0.3 Remove non-runtime checking/probe scripts
...
- [ ] 1. Set up project structure and core value objects
  - [~] 1.1 Create Python project skeleton and environment configuration
  ...
  - [~] 1.2 Define core value objects and enums
...
- [ ] 2. Implement the ClickHouse blackboard persistence layer
  - [~] 2.1 Implement ClickHouse client wrapper and schema creation
  ...
  - [~] 2.3 Implement loop-state read/write with latest-version-wins semantics
...
- [ ] 3. Implement goal validation and loop initiation
  - [~] 3.1 Implement the `validate_goal` pure validator
  ...
  - [~] 3.3 Implement all-or-nothing goal initiation
...
- [ ] 4. Implement the pluggable Scorer interface
  - [~] 4.1 Define the `Scorer` protocol and the `HeuristicScorer` default
  ...
  - [~] 4.2 Implement `PioneerScorer` and `select_scorer`
...
- [ ] 5. Implement the Scanner and signal sources
  - [~] 5.1 Implement the `SignalSource` protocol and the Hacker News Algolia source
  ...
  - [~] 5.2 Implement the GitHub source via the Airbyte_Agent
  ...
  - [~] 5.3 Implement the Hugging Face Hub stretch source
  ...
  - [~] 5.4 Implement the Scanner with dedup/upsert and per-source retry orchestration
...
- [ ] 6. Implement the Qualifier
  - [~] 6.1 Implement scoring with TrueFoundry explanation and placeholder fallback
  ...
  - [~] 6.2 Implement per-candidate Pioneer fallback and threshold forwarding
...
- [~] 7. Run-and-commit checkpoint (after the Qualifier group)
...
- [ ] 8. Implement the Writer
  - [~] 8.1 Implement budget-respecting personalized drafting
...
- [ ] 9. Implement the Governance Gate
  - [~] 9.1 Implement approval and edit-invalidation
  ...
  - [~] 9.2 Implement the `authorize_send` pure decision function
...
- [ ] 10. Implement the Sender interface
  - [~] 10.1 Implement the `Sender` protocol and `SmtpSender` default
  ...
  - [~] 10.2 Implement `GmailAgentSender`, backend selection, and gate-routed sending
...
- [ ] 11. Implement the Optimizer and reply-rate analytics
  - [~] 11.1 Implement outcome collection, storage, and learning feed
  ...
  - [~] 11.2 Implement reply-rate computation (scalar and 24-hour buckets)
